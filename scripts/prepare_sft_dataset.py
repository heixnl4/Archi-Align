...
你的任务是提取【参考文本】中的核心建筑史知识，并模拟“真实求知者”的口吻，动态生成用于训练 RAG 系统的问答对。

【提取与生成规则】（严格遵守）：
1. 绝对禁止使用前缀词：问题（instruction）中绝对不能出现“外国建筑史B课程”、“根据参考文本”、“文中提到”、“课程中”、“教材指出”等字眼。必须直接提问核心知识点。
   - 错误示范：“根据参考文本，现代主义建筑的三条历史线索是什么？”
   - 正确示范：“现代主义建筑的三条历史线索是什么？”
2. 评估信息密度：如果文本整段都在讲闲聊或课程安排（如“今天就上到这里”），请直接返回空数组 `[]`。
3. 动态数量：根据有效知识点数量，生成对应数量的正样本。知识点越密集，生成的正样本越多（数量在 1-4 个左右）
4. 负样本构造：针对文本的核心实体，提出一个具体的、但在文本中未提及细节的迷惑性问题（同样不允许使用“参考文本”等前缀词）。
